...
export const LessonScreen = ({ apiKey, userId, learningMode, difficulty, selectedUnit }) => {
  ...
  const handleQuizComplete = (result) => {
    setScores(prev => ({ ...prev, quizCorrect: result.correct, quizTotal: result.total }));
    setCurrentStep('essay');
  };

  const handleEssayComplete = (result) => {
    setScores(prev => ({ ...prev, essayScore: result.score, nextAction: result.recommended_action }));
        ...
        <Box sx={{ textAlign: 'center', mt: 3, mb: 4 }}>
           ...
           <Typography 
             ...
             sx={{ 
               ...
               mt: 1, 
               lineHeight: 1.4,
               fontSize: { xs: '1.25rem', md: '1.5rem' } // スマホ: 20px, PC: 24px
             }}
           >
